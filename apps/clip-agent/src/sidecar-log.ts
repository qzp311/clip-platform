...
/** Match only explicit Python import/setup failures — not ffmpeg "not installed" hints. */
...
export function isBenignSidecarLine(line: string): boolean {
  ...
  return (
    ...
    /funasr-server listening|GPU backend ready|GPU 后端|预热 GPU|Notice: ffmpeg/i.test(text) ||
